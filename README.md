...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\install.ps1 -Install
```
...
安装器**不会**修改 `.wezterm.lua`，也**不会**调用 WezTerm 或自动 reload。
...
Lua 会校验 pane ID、thread UUID、rollout 路径是否位于 `$CODEX_HOME` 内，以及 rollout 中的 `session_meta` 是否匹配。只有全部通过后才读取 token 数据。
...
本地仓库已经删除时，可以使用已安装的 bridge 先移除 hook：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$HOME\.codex\wezterm-statusline\bin\codex_statusline_bridge.ps1" -Uninstall
```
...
Windows 完整卸载会移除 Lua 模块、bridge 脚本和本插件自己的 `SessionStart` handler，不删除其他 hook、hook 备份或历史 pane mapping。
