...
sidebar_position: 4
title: 工作原理
description: 了解进程识别、pane/thread 映射、rollout 读取和状态 pane 生命周期。
---
...
运行状态升级至 schema 8，按 owner ID 保存绑定和代际；配置 schema 保持 1。旧 schema 7 状态只在能验证原 idle shell 身份时接管，不按相同标题删除其他 pane。SSH 远端进程和 tmux 内部分屏识别不在本次扩展范围内。
