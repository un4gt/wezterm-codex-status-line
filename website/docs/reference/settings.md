...
| `sessions.max_tail_lines` | `200` | 每轮尾部扫描与费用历史扫描各自的最大行数 |
...
费用按每次请求使用的模型分别累计，切换模型不会将过去的用量套用到新模型上。每个模型使用当前配置的单价；覆盖单价后，该模型对应的历史估算也会更新。费用不代表实际账单，不包含长上下文、加速、缓存写入和工具费用。历史记录或模型价格不完整时显示 `Cost —`。
