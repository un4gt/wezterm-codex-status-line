...
| MIT 许可 | 根目录 `LICENSE` | npm 包根目录、Python 源码包根目录与 wheel 的 `.dist-info/licenses/` |
...
不要手工修改两个包的 `assets/`、包根目录的 `LICENSE` 或 `website/static/config.schema.json`。对应同步命令会覆盖这些文件。
...
4. npm tarball、Python wheel 与源码包包含同版本资源、contract 和根目录 MIT LICENSE。
